#!/usr/bin/env bash
# CPU profile of the TUI (localhost pprof). Requires a running TUI with AGENT_TUI_PPROF=1.
set -euo pipefail
ADDR="${AGENT_TUI_PPROF_ADDR:-127.0.0.1:6060}"
...
echo "  AGENT_TUI_PPROF=1 go run ./tui -ui-only"
