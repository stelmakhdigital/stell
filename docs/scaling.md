...
```bash
AGENT_HMAC_KEY=dev-hmac go run ./cmd/runtime -addr 127.0.0.1:8081
AGENT_API_TOKEN=dev-token AGENT_HMAC_KEY=dev-hmac go run ./cmd/gateway -runtime http -runtime-url http://127.0.0.1:8081
```
...
`deploy/k8s/agent.yaml` — 2 runtime + 2 brain, shared PVC for sessions, secrets `hmac` / `api`.
